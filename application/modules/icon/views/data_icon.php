...
$(document).ready(function(){
	...
	table = $("#mydata").dataTable({
		...
		"serverSide": true,
		"ajax": {
			"url": "<?php echo base_url()?>index.php/icon/ajax_list",
			"type": "POST"
		},
		"columnDefs":[{    
			targets: [ -1 ],
...
function reload_table()
{
    ...
    $.ajax({
    	url : "<?php echo site_url('icon/ajax_edit/')?>/" + id,
    	type: "GET",
    	dataType: "JSON",
    	success: function(data)
    	{
            ...
            $('.title').text('Edit Data : '+data.icon); // Set title to Bootstrap modal title
...
{	
	...
	.then((willDelete) => {
		if (willDelete) {
			$.ajax({
				url : "<?php echo site_url('icon/ajax_delete')?>/"+id,
				type: "POST",
				dataType: "JSON",
				success: function(data)
				{
